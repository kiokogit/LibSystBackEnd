...
export const signUpUser = async (req, res) => {
    try {
        ...
        else res.json({status:'error', message:'Server Error'})
    };
};
    ...
    try {
        ...
        if (!user) {
            ...
            if (await bcrypt.compare(password, user.userPassword)) {
                ...
                }, process.env.REFRESH_TOKEN_SECRET, { expiresIn: '5s' });
        ...
        if (e.message.includes('timed out')) res.json({ status: "error", message: 'Resource Time Out' })
        else res.json({ status: 'error', message: e.message })
    }
};
...
export const verifyToken = (req, res, next) => {
    ...
    jsonwebtoken.verify(authHeader, process.env.ACCESS_TOKEN_SECRET, ((err, user) => {
        if (err) {
