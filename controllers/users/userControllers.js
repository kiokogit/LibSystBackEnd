...
export const editProfile = async (req, res) => {
    ...
    const {userPassword, firstName, lastName, userPhone, gender, dateOfBirth, userAddress, profilePhoto, bio, messages, notifications} = req.body

    try {
        const newPassword = await bcrypt.hash(userPassword, 5)
        await User.updateOne({ userEmail: email }, { $set: { userPassword:newPassword, firstName, lastName, userPhone, gender, dateOfBirth, userAddress, profilePhoto, bio, messages, notifications } });
